define(['angularAMD', 'dddvalidate','angularUIrouter','angularResource','bootstrap','bootstrapDatetimepicker','angularFileUpload',
        'bootstrapDatetimepickerCn','angularUIBootstrap','angularAnimate','appPermission','uigrid',
        'DDataGrid','DReference','DSelect','onToManyGrid','dddCompile','MenuCtrl',
        'viewTree','DfileUpload','DimgUpload','ueditor','ddatepicker','formValidate',
        'ExportAndImport','uicalendar','fullcalendar','moment','jquery','w2ui','uiCodemirror',
        'DlogoUpload','cropbox',
        'sanitize','ui.select','DddIntroduce','dialogService','OperatorCongfigService','ReportViewService','ModuleService'
        /* dddinclude start WebContent\ddd\extends\js\app_sub1.js */

	/* dddinclude end WebContent\ddd\extends\js\app_sub1.js */   
        ], function (angularAMD,moment) {
	var app = angular.module("app", ['complieApp','ui.validate','ui.router','ngResource','ui.bootstrap','ui.bootstrap.tpls','ngAnimate',
	                                  'angular.permission','dDataGridApp','dddReferenceApp','ddd.validator','menuControl',
	                                  'dddSelectApp','onToManyGridModule','viewTreeModule','dddFileUploadApp',
	                                  'ui.grid', 'ui.grid.edit', 'ui.grid.resizeColumns','ui.grid.selection','ui.grid.pagination',
	                                  'ng.ueditor','dDatePickerApp','dddImgUploadApp',
	                                  'exportAndImportApp','ui.calendar','ui.grid.treeView','ui.grid.exporter','ui.grid.rowEdit','ui.grid.pinning','ui.grid.resizeColumns',
	                                  'ui.grid.moveColumns','ui.grid.resizeColumns',
	                                  'ngSanitize','ui.select','ui.grid.autoResize','dddLogoUploadApp',
	                                  'ui.codemirror','moduleModule','operatorCongfigModule','reportViewModule'
	                                  /* dddinclude start WebContent\ddd\extends\js\app_sub2.js */

 /* dddinclude end WebContent\ddd\extends\js\app_sub2.js */                               
	                                  ]);
	app.run(function($rootScope, $state, $stateParams,angularPermission,OperatorCongfigService,ModuleService) {
		$rootScope.$state = $state;
		$rootScope.$stateParams = $stateParams;
		$rootScope.$state.isLogin = false;
		$rootScope.showLeftContent = true;
		
		$rootScope.changeLeftContentDisplay = function(){
			$rootScope.showLeftContent = !$rootScope.showLeftContent;
		}
		angularPermission.registerPersistentChache('operatorCongfig',OperatorCongfigService.findOneOperatorCongfig,OperatorCongfigService.saveOperatorCongfig);
		angularPermission.registerPersistentChache('systemChildModule',ModuleService.findSystemChildModulesNew1,null);
		...
		return {
			      restrict : 'A',
			        link: function(scope, element, attrs, ngModelController) {
			        	 var ddd = scope.$eval(attrs.ngRightClick);
			 	        element.bind('contextmenu', function(event) {
			 	            scope.$apply(function() {
			 	                event.preventDefault();
			 	                $rootScope.tabMenuController[ddd.eId] = true;
			 	                //alert(ddd.eId);
			 	            });
			 	        });		
			        },
		...
		var httpInterceptor = {
			'responseError' : function(response) {
				if (response.status == 401) {
					...
					var stateParams = rootScope.$state.params;
					if(state.indexOf("login") < 0)
					{
						rootScope.stateBeforLogin = state;
						rootScope.stateParamsBeforLogin = stateParams;
						alert("登陆过时!");
			...
			'request':function(config){
				var rootScope = $injector.get('$rootScope');
				var hash = location.hash;// 获取锚点 
				/*if (hash =="#/login" || hash =="") {
					config.url = config.url + "?isLogin=1";
				}*/
				if (localStorage.length <= 2 && hash !="#/login" && hash !="") {
					 //2017-3-3 linyuan 解决请求url不变只是锚点变化引起的问'
					location.hash = "#/login";
					rootScope.$state.go("login");
				}
				return config;
	...
	}]).config([ 'angularPermissionProvider', function(angularPermissionProvider) {
		
		//静理'?化
		var jsonCache = require("jsonCache").cached;
		angularPermissionProvider.initCache(jsonCache);
	...
	app.config(['$resourceProvider', 'angularPermissionProvider',function($resourceProvider,angularPermissionProvider) {
		angular.forEach($resourceProvider.defaults.actions,function(action,key){
			
			action.interceptor={
					'response' : function(response,angularPermission) {
						var datatype = response.headers("datatype");
						if(datatype === "object")
						{
							return response.resource;
						}
						else if(datatype === "objectWithReference")
						{
							 
							var objectWithReference = angularPermissionProvider.fastjson.format(response.resource);
						...
						else if(datatype === "arrayWithReference")
						{
							var arrayWithReference = angularPermissionProvider.fastjson.format(response.resource);
							
							return arrayWithReference;
						}						
	...
	app.config(function($stateProvider, $urlRouterProvider,$rootScopeProvider) {
		...
		/* dddinclude start WebContent\ddd\extends\js\appRouter.js */
                 ...
                 'courseDesignList@main.list' : angularAMD.route({
                	 controller:'courseDesignListController',
 					 controllerUrl : ['simple/view/courseDesign/js/courseDesign/CourseDesignList.js',
 						 			  'simple/view/task_module/js/task/TaskService.js'],
            ...
            views : {
                'courseDesignDisplay@main.list' : angularAMD.route({
					...
					controllerUrl : ['simple/view/courseDesign/js/courseDesign/CourseDesignDisplay.js',
									 '/DDD3.2/ddd/simple/view/task_module/js/task/TaskService.js',
									 '/DDD3.2/ddd/simple/view/teacher_module/js/teacher/TeacherService.js',
									 '/DDD3.2/ddd/simple/view/courseDesign/js/courseDesignProgress/CourseDesignProgressService.js',
									 '/DDD3.2/ddd/simple/view/organization/js/employee/EmployeeService.js',
									 '/DDD3.2/ddd/simple/view/student_module/js/student/StudentService.js'],
            ...
            views : {
                'courseDesignProgressDisplay@main.list' : angularAMD.route({
					...
					controllerUrl : ['simple/view/courseDesign/js/courseDesignProgress/CourseDesignProgressDisplay.js',
									 'simple/view/courseDesign/js/courseDesign/CourseDesignService.js'],
                 ...
                 'stuChooseCourseList@main.list' : angularAMD.route({
                	 controller:'stuChooseCourseListController',
 					 controllerUrl : ['simple/view/courseDesign/js/stuCourseProgress/stuChooseCourseList.js',
 						 			  'simple/view/courseDesign/js/courseDesign/CourseDesignService.js',
 						 			  'simple/view/organization/js/employee/EmployeeService.js',
 						 			  'simple/view/teacher_module/js/teacher/TeacherService.js'],
                 ...
                 'personalDataForm@main.list' : angularAMD.route({
                	 controller:'personalDataFormController',
 					 controllerUrl : ['simple/view/personalCenter/js/personalData/PersonalDataForm.js',
 						 			  'simple/view/teacher_module/js/teacher/TeacherService.js',
 						 			  'simple/view/student_module/js/student/StudentService.js'],
...
//end  ====D:\angular\workspace\DDD3.2\WebContent\ddd\simple\view\ws\js\zldj\Zldj.route.js
	...
	/* dddinclude end WebContent\ddd\extends\js\appRouter.js */
	})
	/**
	 *  TAB页处'
	 *  放在外面会因为加载顺序的问题导致无法同时存在多个模块的TAB'
	 */

	
	app.run(function($rootScope,$state,$timeout,$location){
//		if(window.sessionStorage){
//			$rootScope.tabs = angular.fromJson(sessionStorage.getItem("tabs")) || [];
//		}else{
//			$rootScope.tabs = [];
//		}
		$rootScope.tabs = [];
		$rootScope.isContextMenuOpen = false;
		...
		/** 关闭当前活动的Tab' */
		$rootScope.closeOperateTab = function() {
			var tabs = $rootScope.tabs;
			for (var i = tabs.length-1; i >=0; i--) {
				if (tabs[i].active == true) {
					$rootScope.closeTabByEId(tabs[i].eId);
					break;
				}
			}
		};
		...
		/** 关闭非活动的Tab' */
		$rootScope.closeUnoperateTab = function(tab) {
			var tabs = $rootScope.tabs;
			var needClose = [];
			for (var i =0,j=tabs.length;i<j;i++) {
				if(tabs[i].eId == tab.eId){
					//不做
				}else{
					needClose.push(tabs[i].eId);
				}
			
			}
			for(var i = 0,j=needClose.length;i<j;i++){
				$rootScope.closeTabByEId(needClose[i]);
			}
//			$rootScope.selectTab(tab);
		};
		
		$rootScope.selectTab = function(selectedTab){
			var isFind = false;
			angular.forEach($rootScope.tabs,function(tab,index,tabs){
				tab.active = false;
				if( tab === selectedTab || tab.eId === selectedTab){
					tab.active = true;
					isFind = true;
				}
			})
		...
		/** 关闭全部的Tab' */
		$rootScope.closeAllTab = function() {
			var tabs = $rootScope.tabs;
			for (var i = tabs.length-1; i >=0; i--) {
				$rootScope.closeTabByEId(tabs[i].eId);
			}
		...
		/** 关闭当前活动的Tab页左侧的Tab' */
		$rootScope.closeLeftTab = function(tab) {
			var tabs = $rootScope.tabs;
			var needClose = [];
			for (var i = 0; i<tabs.indexOf(tab); i++) {
				needClose.push(tabs[i].eId);
			}
			for(var i = 0,j=needClose.length;i<j;i++){
				$rootScope.closeTabByEId(needClose[i]);
		...
		/** 关闭当前活动的Tab页右侧的Tab' */
		$rootScope.closeRightTab = function(tab) {
			var tabs = $rootScope.tabs;
			var needClose = [];
			for (var i = tabs.indexOf(tab)+1; i<tabs.length; i++) {
				needClose.push(tabs[i].eId);
			}
			for(var i = 0,j=needClose.length;i<j;i++){
				$rootScope.closeTabByEId(needClose[i]);
		...
		/**通过EID关闭tab页并跳转路由*/
		$rootScope.closeTabByEIdWithState = function(EId){
			//关闭后跳转路由操'
			//1.关闭活动页理'?理'?跳转到上一个tab页的路由
				//直接跳转到tabs中的'后一个tab页的路由
			//2.关闭非活动页—理'?不做路由跳'
			var tab = $rootScope.getTabByEId(EId);
			$rootScope.closeTabByEId(EId);
			if(tab.active == true){
				$rootScope.selectTab($rootScope.tabs[$rootScope.tabs.length-1]);
//				var url = $rootScope.tabs[$rootScope.tabs.length-1].url;
//				 $timeout(function(){ $location.path(url)},10);
			}else{
				//关闭非活动页不做操作
			}
		}
		$rootScope.getTabByEId = function(EId){
			for (var i = 0, count = $rootScope.tabs.length; i < count; i++) {
				if ($rootScope.tabs[i].eId == EId) {
					return $rootScope.tabs[i];
				}
			}
		}
		/** 通过Eid关闭Tab' */
		$rootScope.closeTabByEId = function(EId) {
			var index = 0;// 记录当前元素下标
			angular.forEach($rootScope.tabs, function(tab) {
				if (tab.eId === EId && tab.eId !== "mainTable") {
					$rootScope.tabs.splice(index, 1);// 删除当前下标的理'?项卡元'
				}
				index++;
			});
			if(window.sessionStorage){
				sessionStorage.setItem("tabs",angular.toJson($rootScope.tabs));
			}
			if($rootScope.tabs.length == 0){
				$state.go("main.list.mainTable");
			}
		};

		/** 通过Eid找到Tab' */
		$rootScope.findTabByEId = function(EId) {
			for (var i = 0, count = $rootScope.tabs.length; i < count; i++) {
				if ($rootScope.tabs[i].eId == EId) {
					return true;
				}
		...
		$rootScope.tabMenuController = {};
		
		var addTab = function(model,stateParams){
			if($rootScope.tabs.length > 10){
				alert("当前打开Tab页过多，请关'");
			}else{
				setAllInactive();
				
				if(model.code == null || model.code == ''){
					model.code = model.eId;
				}
				//解决动理'?表单同'路由'要跳多个tab页的问题，改掉现有tab页所有code
				if($rootScope.tabs && $rootScope.tabs.length > 0){
					for(var i = 0 ; i < $rootScope.tabs.length ; i++){
						if($rootScope.tabs[i].code){
							$rootScope.tabs[i].code = $rootScope.tabs[i].code + Date.now(); 
						}
					}
				}
				...
				$rootScope.tabMenuController[model.eId] = false;
			}
		};
		
		$rootScope.selectModule = function(model,stateParams) {
			var moduleEId = model.eId;
			var hasTab=false;
			var tabs = $rootScope.tabs;
			for(var i=0;i<tabs.length;i++)
			{
				if(moduleEId==tabs[i].eId)
				{
					setAllInactive();
					tabs[i].active = true;
					hasTab=true;
