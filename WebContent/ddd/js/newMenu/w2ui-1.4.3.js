        ...
        refresh: function (id) {
            ...
            function getNodeHTML(nd) {
                ...
                if (nd.group) {
                    ...
                    html =
                    '<div class=" '+(nd.nodes.length > 0 ? (nd.expanded ? 'w2ui-expand-ddd' : '') : '')+'  w2ui-node '+ (nd.selected ?'w2ui-selected': '') +' '+ (nd.disabled ? 'w2ui-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
                        '    onClick="w2ui[\''+ obj.name +'\'].dblClick(\''+ nd.id +'\', event);w2ui[\''+ obj.name +'\'].click(\''+ nd.id +'\', event);"'+
                        ...
                        '        if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                        '    <div class="w2ui-expand">'    + (nd.nodes.length > 0 ? (nd.expanded ? '<span class="glyphicon glyphicon-menu-down"></span>' : '<span class="glyphicon glyphicon-menu-left unexpend"></span>') : (nd.plus ? '<span class="glyphicon glyphicon-menu-left unexpend"></span>' : '')) + '</div>' +
                        '</td>'+
                        '</tr></table>'+
                    '</div>'+
                    '<div class="w2ui-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
