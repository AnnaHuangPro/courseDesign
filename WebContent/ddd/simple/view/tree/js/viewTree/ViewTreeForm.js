...
function() {
    return angular.module('viewTreeModule', []).controller('viewTreeFormController', ['$rootScope', '$stateParams', '$scope', '$state', 'ViewTreeService', 'dialogs',
    function($rootScope, $stateParams, $scope, $state, ViewTreeService, dialogs) {
        ...
        $scope.refreshNodeGrid = function() {
            $rootScope.getController('treeNodeGrid', 'ddatagrid').setDataGridProviders($scope.viewTree.viewTreeNodes);
        }
        ...
        $scope.alertNodeDialog = function(node) {
            ...
            dlg.result.then(function(viewTreeNode) {
                ...
                } else if(viewTreeNode.isNew == true){
                	viewTreeNode.isNew = false;
                    $scope.viewTree.viewTreeNodes.push(viewTreeNode);
                }else{
                	for (var i = 0; i < $scope.viewTree.viewTreeNodes.length; i++) {
                        if ($scope.viewTree.viewTreeNodes[i].createDate == viewTreeNode.createDate) {
                            $scope.viewTree.viewTreeNodes[i] = viewTreeNode;
                            break;
                        }
                    }
    ...
    function(data, $rootScope, $scope, $uibModalInstance) {
        ...
        $scope.save = function() {
        	if(!$scope.viewTreeNode.eId && $scope.viewTreeNode.isNew == undefined){
        		$scope.viewTreeNode.isNew=true;
        		$scope.viewTreeNode.createDate = (new Date()).getMilliseconds();;
        	}
