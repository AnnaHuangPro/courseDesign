...
.controller("assignPermissionsController",['$scope','$stateParams','PermissionService','RoleService','ModuleService','$state','angularPermission',function($scope,$stateParams,PermissionService,RoleService,ModuleService,$state,angularPermission){
	$scope.ModuleTree = [];
	...
	{
		if(data.code == angularPermission.getLoginUser().operatorAndRoles){
			//sessionStorage.removeItem("permissions");
			angularPermission.removePermissions();
			//sessionStorage.setItem("permissions",data.permissions);
			angularPermission.setPermissions(data.permissions);
		//	console.log(sessionStorage.getItem("permissions"));
		}
	}
	$scope.saveCurrentSuccessFn = function(data){
		var roles = angularPermission.getRoles();;
		for(var i =0;i< roles.length;i++){
			if(data.code == roles[i].code){
				...
				//sessionStorage.removeItem("permissions");
				angularPermission.removePermissions();
				angularPermission.setPermissions(permissionsStr);
	...
	$scope.toggleSelect = function(permission){
