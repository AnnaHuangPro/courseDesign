...
public class OperatorDaoBean extends BaseDao implements OperatorDao {
	...
	@Override
	public Operator findOperatorByUserKey(String userKey) throws Exception {
		String sql="select * from operator op "
				+ " left join employee emp on op.employeeId=emp.EId"
				+ " WHERE emp.userKey='"+userKey+"'";
		EntitySet<Operator> operators= this.queryBySql(sql, Operator.class);
		
		if(operators.size()>0){
			return (Operator) operators.toArray()[0];
		}
		
		return null;
	}
	
	public Operator findOperatorByEmployeeId(Long employeeId) throws Exception
	{
		String whereSql = "employeeId="+employeeId;
	    return this.queryOne(whereSql, Operator.class);
	}
