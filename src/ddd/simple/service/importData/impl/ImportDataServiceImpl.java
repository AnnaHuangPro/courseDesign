...
public class ImportDataServiceImpl implements ImportDataService {
	...
	public List<CellError> importData() {
		...
		//	ImportHandler importHandler = this.config.getImportHandler();
			...
			if (!"".equals(outInterface) && outInterface != null && this.cellErrors.size() == 0) {// 外部参数
	...
	private Map<Integer, String> readHead(Sheet sheet){
		...
		if (this.columnTitles == null) {
			...
			for (int j = 0; j < sheet.getColumns(); j++)// 读取表头
			{
				...
				ConfigItem configItem = this.config.getConfigItemByTitle(title.trim());
	...
	private int readBody(Sheet sheet,Map<Integer, String> emptyIndexs) throws Exception{
		...
		for (int i = 1; i < sheet.getRows(); i++) {
			...
			temp.putAll(emptyIndexs);
			
			int emptys = 0,cellIndex=0;// 记录每一行空单元格的个数
			for (int j = 0; j < configItems.size(); j++) {
  				if(temp.containsKey(j)){
  					temp.remove(j);
					continue;
				}
				ConfigItem configItem = this.configItems.get(cellIndex);
				if (configItem != null) {
					Object value = this.getContents(sheet.getCell(j, i), configItem, rowIndex);
					...
					if (value == null) {
					...
					}else if (configItem.getIsUnique()) {// 唯一
						...
						where +="and "+ this.getCorrectSqlWord(configItem, value, configItem.getFieldName());
					...
					if (configItem.getIsEntity()) {
						...
						if (!className.contains("EntitySet")) {
							data.put(this.fieldNames.get(cellIndex), value);
						}
					} else {
						data.put(this.fieldNames.get(cellIndex), value);
					}
	...
	public Long getExistEid(Map<String, Object> data, int rowIndex) throws Exception {
		...
		for (ConfigItem configItem : config.getConfigItems()) {
			if (configItem.getIsUnique()) {
				...
				if(data.containsKey(data.get(name))){
					where +="and " + this.getCorrectSqlWord(configItem, data.get(name), name);
				}
			}
		}
	 ...
	 * @return
	 */
	private String getCorrectSqlWord(ConfigItem configItem,Object value,String columnName){
		String result = "";
		
		Class<?> fieldType=configItem.getColumnInfo().getFieldType();
		
		if(fieldType.equals(String.class)){
			if(configItem.getColumnInfo().getLength() <= 255){
				result=columnName+" = '"+value.toString()+"'";
			}else {//clob 字段
				result="dbms_lob.instr("+columnName+",'"+value+"',1,1) > 0";
			}
		}else if(fieldType.equals(Date.class)){
			result="TRUNC(" + columnName+ ") = TRUNC(to_date('"+ DateUtil.formatDate((Date) value) + "','yyyy-MM-dd'))";
