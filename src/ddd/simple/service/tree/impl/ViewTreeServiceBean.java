...
public class ViewTreeServiceBean implements ViewTreeService {
	...
	public EntitySet<ViewTree> findAllViewTree(){
		try {
			return this.viewTreeDao.findAllViewTree();
		} catch (Exception e) {
			e.printStackTrace();
			throw new DDDException("findAllViewTree", e.getMessage(), e);
	...
	public Set<Map<String, Object>> findDataByViewTreeNodeIndex(Long viewTreeId, Map node, Map initFilter) {
		try 
		{
			...
			Set<Map<String, Object>> results = this.getDataFromViewTreeNode(viewTreeId, node,initFilter);
			if(results == null && issHierachical.equals("true"))
			{
				node.put("issHierachical", "false");
				results = this.getDataFromViewTreeNode(viewTreeId, node,initFilter);
			}
			return results;
	...
	private Set<Map<String, Object>> getDataFromViewTreeNode(Long viewTreeId,Map node,Map initFilter)
	{
		try
		{
			int nodeIndex = 0;
