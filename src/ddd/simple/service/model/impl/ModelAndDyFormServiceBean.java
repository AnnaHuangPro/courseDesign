...
public class ModelAndDyFormServiceBean implements ModelAndDyFormService
{
		...
		while (iterator.hasNext())
		{
			...
			if (modelItem.getDatatype() != null)
			{
				...
				} else if (modelItem.getIsUse().equals("是"))
				{
					...
					JSONObject relatedParameters = JSONObject.parseObject(modelItem.getRelatedParameters() == null ? "" : modelItem.getRelatedParameters());
					String dataType = modelItem.getDatatype();
					if ("radio".equalsIgnoreCase(dataType)||"checkbox".equalsIgnoreCase(dataType)||"select".equalsIgnoreCase(dataType))
					{
						this.handleChoose(relatedParameters);
					}
					...
					config.put("controlId", "fb-" + String.valueOf(Math.random()).substring(2));
					configs.add(config);
				}
			} else
			{
	...
	public void handleChoose(JSONObject relatedParameters)
	{
		String dataSource = relatedParameters.getString("dataSource").replaceAll("[\"()\\[\\]]", "");
		String[] dataSourceArry = dataSource.split("\n|,");
		for (int i = 0; i < dataSourceArry.length; i++) 
		{
			dataSourceArry[i] = dataSourceArry[i].replace("，", "\",");
			dataSourceArry[i] = dataSourceArry[i].replace("选项名：", "\"name\":\"");
			dataSourceArry[i] = dataSourceArry[i].replace("选项值：", "\"value\":\"");
			dataSourceArry[i] = dataSourceArry[i].replace("默认选项：", "\"isDefaultValue\":\"");
			dataSourceArry[i] = dataSourceArry[i].replace("是", "true\"");
			dataSourceArry[i] = dataSourceArry[i].replace("否", "false\"");
			dataSourceArry[i] = "{"+dataSourceArry[i]+"}";
		}
		List<Map<String,Object>> dataSourcelist = new ArrayList<Map<String,Object>>();
		for(String data:dataSourceArry)
		{
			dataSourcelist.add((Map<String, Object>)JSONObject.parseObject(data));
		}
		relatedParameters.put("dataSource", dataSourcelist);
		
		relatedParameters.put("dataSourceString",Arrays.toString(dataSourceArry).replace("\"", "&quot;"));
	}
	...
	private void dealSubItem(ModelItem item) throws Exception
	{
		...
		if (displayFields != null && !"".equals(displayFields))
		{
			...
			JSONArray operationColumns = createSubTableOperationColumns();
	...
	private JSONArray createSubTableOperationColumns()
	{
	...
	private JSONArray createSubTableOperationBarButtons(Model subModel)
	{
		...
		JSONObject displayColumn2 = new JSONObject();
		displayColumn2.put("label", "选择数据");
		displayColumn2.put("operate", "select");
		displayColumn2.put("template", "simple/view/model/html/modelData/SelectSubData.html");
		displayColumns.add(displayColumn2);
