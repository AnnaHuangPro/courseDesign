...
public class ModelTypeServiceBean   implements ModelTypeService
{
	...
	{
		try {
			Long eid = modelType.getEId();
			if(eid != null && this.modelTypeDao.findModelTypeById(eid) != null){
				modelType = this.modelTypeDao.updateModelType(modelType);
			}
			else{
				modelType = this.modelTypeDao.saveModelType(modelType);
			}
			return modelType;
		} catch (Exception e) {
			e.printStackTrace();
