...
public class PermissionServiceBean extends BaseService implements PermissionService {
	...
	public EntitySet<Permission> savePermissions(EntitySet<Permission> permissions){
		try {
			return  (EntitySet<Permission>) this.permissionDao.save(permissions);
		} catch (Exception e) {
			e.printStackTrace();
			throw new DDDException("savePermission", e.getMessage(), e);
		}
	...
	public EntitySet<Permission> findShowPermissions(Long roleId,Long moduleId) throws Exception{
		try {
			...
			{
				modulePermission.setIsAssigned(false);
			}
	...
	public int deletePermissionByCode(String code) throws Exception{
		return this.permissionDao.deletePermissionByCode(code);
	}
	...
	public int getPermissionCount(){
		try
		{
			return this.permissionDao.getPermissionCount();
		} catch (Exception e)
		{
			e.printStackTrace();
			throw new DDDException("getPermissionCount", e.getMessage(), e);
		}
	}
