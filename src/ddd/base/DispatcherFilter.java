...
public class DispatcherFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		System.out.println("" + new String(
				JSON.toJSONString(request.getParameterMap(), true).getBytes(Config.CHARSET_ISO), Config.CHARSET_UTF8));
		System.out.println("--------------------------------------");

		String ddd = request.getParameter("isLogin");
		/* if (!"1".equals(ddd)) {/ */
		try {
			// 判断是否登录
			Boolean isLogin = noLoginFilter(httpServletRequest, currentRequestPath);
			if (!isLogin && !currentRequestPath.endsWith("index.html") && isHTMLRequest(currentRequestPath)
					&& !currentRequestPath.endsWith("LoginModule.html")) {
				throw new Exception("未登录");
			}
		} catch (Exception e) {
			ClientError clientError = ClientError.create(new Throwable("未登录"), currentRequestPath,
					ClientError.createNewId());
			...
			WebUtil.writeJSON(httpServletResponse, clientError);
	...
	// loginUser = (LoginUser)
	// ((HashMap)httpServletRequest.getSession().getAttribute("loginUser")).get("loginUser");
	// }
	//
	...
	// 处理action请求
	private boolean actionHandler(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
			String requestPath) {
	...
	private Map<String, String> getBodyParameters(HttpServletRequest request) {
		...
		Map<String, String> bodyParameters = JSON.parseObject(body.toString(),
				new TypeReference<Map<String, String>>() {
				});
		return bodyParameters;
	}

	private Object[] createActionMethodParamList(HttpServletRequest request, HttpServletResponse response,
	...
	private boolean isStaticResource(String currentRequestPath) {
		String staticResourceReg = ".+\\.((html)|(htm)|(jsp)|(png)|(gif)|(jpg)|(swf)|(css)|(js)|(map))$";
	...
	private boolean noLoginFilter(HttpServletRequest request, String currentRequestPath) {
		...
		if (session == null
				|| (session.getAttribute("loginUser") == null && session.getAttribute("vipLogin") == null)) {
			/**
	...
	private boolean isHTMLRequest(String currentRequestPath) {
		String staticResourceReg = ".+\\.((html)|(htm)|(jsp))$";
		Matcher requestPathMatcher = Pattern.compile(staticResourceReg).matcher(currentRequestPath);
