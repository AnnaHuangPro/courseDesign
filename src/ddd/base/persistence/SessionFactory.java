...
import ddd.base.dataInit.imp.ModuleDataInit;
import ddd.base.dataInit.imp.OrgDataInit;
import ddd.base.dataInit.imp.OtherDataInit;
import ddd.base.dataInit.imp.PermissionDataInit;
...
import ddd.base.dbmanager.tableHandler.sqlserver.SqlServerTableHandler;
...
import ddd.base.dynamicSql.sqlserver.SqlServerSqlHandler;
...
public class SessionFactory {
	...
	public static void dataInit(){
		//如果权限点这张表中的记录 
		PermissionService permissionService = (PermissionService) ContextLoaderListener.getCurrentWebApplicationContext().getBean("permissionServiceBean");
	...
	private static void initData(){
		...
		{
			//初始化组织机构
			DataInit orgDataInit = new OrgDataInit();
			orgDataInit.init();
			Organization organization = orgDataInit.getOrganization();
			Employee employee = orgDataInit.getEmployee();
			//初始化权限
			DataInit permissionDataInit = new PermissionDataInit();
			permissionDataInit.initParam(organization, null,employee);
			permissionDataInit.init();
			Role role = permissionDataInit.getRole();
			//初始化模块
			DataInit moduleDataInit = new ModuleDataInit();
			moduleDataInit.initParam(null, role, null);
			moduleDataInit.init();
			//初始化其他数据——码表，系统参数
			DataInit otherDataInit = new OtherDataInit();
			otherDataInit.init();
	...
	public static void initDbAndPK(){
		...
		try {
			...
			properties=loader.load("jdbc.properties");
			
			if(properties.containsKey("initDbAndPK")){
				if(properties.get("initDbAndPK").equals("true")){
	...
	public static TableHandler getTableHandler(){
		if(tableHandler==null){
			if(SessionFactory.isMysql()){
				...
				tableHandler = new SqlServerTableHandler();
			}
		}
		return tableHandler;
	}
	public static SqlHandler getSqlHandler(){
		if(sqlHandler==null){
			if(SessionFactory.isMysql()){
				...
				sqlHandler = new SqlServerSqlHandler();
			}
		}
		return sqlHandler;
	}
...
//		}
	...
	* @return EntityClass<? extends Entity> ,如果没有找到实体，则返回 null
	...
	public static boolean isSqlServer(){
		return dataBase.toLowerCase().equals("sqlserver");
	}
