...
public class ModuleAction {
	...
	public EntitySet<Module> test(){
		try
		{
			...
			while(ite.hasNext()){
				...
				module.getName();
				EntitySet<Permission> permissions = this.permissionService.findPermissionByModule(module.getEId());
				Iterator<Permission> ite1 = permissions.iterator();
				while(ite1.hasNext()){
					Permission p = ite1.next();
					p.setEId(null);
					p.setLazyFieidsLoaded(null);
					p.setModule(null);
				}
				...
				module.setLazyFieidsLoaded(null);
			}
			return modules;
		} catch (Exception e)
		{
	...
	public ModuleTreeNode constructNewTree(Long operatorId)throws Exception{
		return this.moduleService.constructNewTree(operatorId);
	}
