...
import ddd.simple.service.workflow.ProcessDefinitionDiagramCmd;
...
public class WorkFlowAction {
	...
	@Resource(name = "processDefinitionDiagramCmd")
	private ProcessDefinitionDiagramCmd processDefinitionDiagramCmd;
	...
	{
		return dynamicFormService.findOwnerTaskList(startPage,pageSize);
	}
 ...
 * @throws Exception 
	...
	{
		try {
			return dynamicFormService.submitTask(taskId, entityVar, parameterMap, taskDetailInfo);
	...
	{
		return dynamicFormService.findNonHandleTasksByCondition(startPage,pageSize,searchCondition);
	}
	
	public Map<String,Object> findOperatedTasksByCondition(int startPage, int pageSize, Map<String,Object> searchCondition)
	{
		return dynamicFormService.findOperatedTasksByCondition(startPage,pageSize,searchCondition);
	}
	
	public Map<String,Object> findCompletedTaskByCondition(int startPage, int pageSize, Map<String,Object> searchCondition)
	{
		return dynamicFormService.findCompletedTaskByCondition(startPage,pageSize,searchCondition);
	}
	
	public byte[] getProcessImage(String processInstanceId,String newDate)
	{
		try
		{
			byte[] images = this.processDefinitionDiagramCmd.getProcessImage(processInstanceId);
			return images;
		}
		catch(Exception e)
		{
	...
	public boolean stopTask(String taskId){
		try {
			return this.dynamicFormService.stopTask(taskId);
		} catch (DDDException e) {
			throw e;
		}
	} 
	...
	{
		try{
			return this.dynamicFormService.findModelsAndAuditedEntitiesName();
		}
		catch(DDDException e){
			throw e;
		}
