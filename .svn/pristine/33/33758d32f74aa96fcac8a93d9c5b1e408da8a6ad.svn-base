...
public class WorkFlowEngineService extends BaseService{
	...
	public String startProcess(String processConfigName,long formId,String tableName,String checkResultProcessVariableName,String description,Map<String,Object> params)
	{
		...
		if(!StringUtils.isBlank(processKey)){
			...
			//工作流唯一标示;版本号;工作流需要的数据源;路由;重新提交路由;终审;
			//process;;employeeCode;main.list.workflow.treeWorkFlow;checkResult
			String[] processConfigs = processConfig.split(";");
			if(processConfigs.length != 6 )
			{
				throw new DDDException("系统参数" + processConfig+ "的格式不正确");
			}

			processDefinitionKey = processConfigs[0];
			processDefinitionVersion= processConfigs[1];
			dataSourceCode = processConfigs[2];
			formRoute = processConfigs[3];
			reApplyRoute = processConfigs[4];
			if(checkResultProcessVariableName == null || checkResultProcessVariableName.length() == 0)
			{
				checkResultProcessVariableName = processConfigs[5];
			}
		}
		return this.startProcess(processDefinitionKey,processDefinitionVersion,dataSourceCode,formId,tableName,formRoute,reApplyRoute,checkResultProcessVariableName,description,params);
	}
	...
	private String startProcess(String processDefinitionKey,String processDefinitionVersion,String dataSourceCode
			,long entityId,String tableName,String formRoute,String reApplyRoute,String checkResultProcessVariableName,String description,Map<String,Object> params)
	{
		...
		try
		{
			...
			{
				if(sqlParams.get("entityId") == null)
				{
					sqlParams.put("entityId", entityId);
				}
				DataSource dataSource = this.dataSourceService.findDataSourceByCode(dataSourceCode);
				Set<Map<String, Object>> dataSourceDatas = this.dataSourceService.getDataSourceResult(dataSource, sqlParams);
			...
			if(loginUser == null){
				...
				operator = this.operatorService.findOperatorByEmployeeId(employeeId);
			}else{
				operator = this.getLoginUser().getLoginOperator();
				member = this.getLoginUser().getLoginVip();
			}
			...
			initParocessVariables.put("申请时间", dateString);
			...
			initParocessVariables.put("重新申请表单路由",reApplyRoute);
			...
			//保存流程启动信息
			this.saveStartProcessInfo(entityId,tableName,code,processStartTime,processInstance.getProcessInstanceId());
	...
	private void saveStartProcessInfo(Long entityId,String entityName, String startProcessPeople,
			Date processStartTime,String processInstanceId){
		String processDefinitionName = this.getProcessName(processInstanceId);
		
		CheckOption checkOption = new CheckOption();
		checkOption.setTaskName(getApplyPerson() + "提交成功");
		checkOption.setCheckTime(processStartTime);
		checkOption.setCheckResult("启动" + processDefinitionName + "成功");
		...
		checkOption.setDescription("启动" + processDefinitionName);
	...
	private String getApplyPerson()
	{
		...
		String applyPerson = operator==null?member.getName():operator.getEmployee().getName();
		return applyPerson;
	}
