...
public class ImportConfigsServiceBean extends BaseService implements ImportConfigsService
{
	...
	private boolean createExcelForConfigs(ImportConfigs savedImportConfigs){
		try{
			...
			String entityId = String.valueOf(savedImportConfigs.getAttachmentCode());
			String entityName = "导入excel模板";
	...
	private File createExcelFile(String entityName,String entityId,ImportConfigs importConfigs){
		EntitySet<Attachment>  attachments = this.attachmentService.findAttachmentByForm(entityId, entityName);
	...
	public void createAllExcelTemplate() {
		try {
			...
			while(iterator.hasNext()){
				ImportConfigs imConfigs=iterator.next();
				
				this.createExcelTemplate(imConfigs);
			}
		} catch (Exception e) {
			e.printStackTrace();
