	...
	return{
		...
		controller:function($scope,$transclude,$state,$element,$parse)
		{
			...
			function findListViewResult(reportView)
			{
				...
				var allCanSeeColumnWidth = 0;
				var bSelectAndOperateWidth = 0;
				//设置除操作列所有列宽
					...
					for(var i=0; i < attributes.length;i++)
					{
						...
						if(angular.isDefined(columnConfig))
						{
							 ...
							 if(columnConfig.visible) {
								 allCanSeeColumnWidth += columnConfig.width;
							 }
						}
						else
						{
				...
				if($scope.viewTreeConditions.length > 0) {
					gridWidth = Math.ceil(gridWidth * 0.834);
				}
				...
				if($scope.operationColumns&& $scope.operationColumns.length > 0)
				{
					...
					bSelectAndOperateWidth = gridWidth - allOperatorWidth - 30;
				...
				for(var i=0;i<attributes.length;i++)
				{
					...
					var colStyle;
					if($scope.extendsColStyle){
						colStyle = $scope.extendsColStyle[column.field];
					}
					if(colOperate){
						var myCellTemplate =$scope.cellTemplate==null?"<div  class=\"ui-grid-cell-contents\" title=\"TOOLTIP\" style="
												+cssArrtribute[1].replace(/'/g,"")
												+"><a href=\"javascript:void(0)\" ng-click=\"grid.appScope.columnClick(row,$index,'"
												+colOperate.colName
												+"')\">{{COL_FIELD}}</a></div>":$scope.cellTemplate;
					}
					//颜色扩展 ：通过配置ngclass
					else if(colStyle){
						var ngClass=" ng-class=\"{";
						for(var style in colStyle.styles){
							var str = colStyle.styles[style].replace("value",":COL_FIELD")
							ngClass+= style+str+",";
						}
						ngClass+="}\"";
						var myCellTemplate = $scope.cellTemplate==null?"<div  class=\"ui-grid-cell-contents\" title=\"TOOLTIP\" style="
								+cssArrtribute[1].replace(/'/g,"") 
								+ngClass+" >{{COL_FIELD}}</div>":$scope.cellTemplate;
					}
					else{
						var myCellTemplate = $scope.cellTemplate==null?"<div class=\"ui-grid-cell-contents\" title=\"TOOLTIP\" style="
												+cssArrtribute[1].replace(/'/g,"")
			...
			/*		var defaultCellTemplate = "<div class=\"ui-grid-cell-contents\" "
				...
				var isFullColumn = bSelectAndOperateWidth > allCanSeeColumnWidth;
				
				if(isFullColumn || reportView.autoFit==undefined||reportView.autoFit == "false"||reportView.autoFit==""){
					...
					var perColumnWidth = ((gridWidth- allOperatorWidth)-30)/canSeeColumnNumber + 1;
					//自适应调整之后不需要限定130了 ----lc
		    ...
		    function initGrid()
		    {
		    	$scope.DefultPageTmplate="<div class=\"ui-grid-pager-panel\" ui-grid-pager ng-show=\"grid.options.enablePaginationControls\"><div class=\"ui-grid-pager-container\"><div class=\"ui-grid-pager-control\"><button type=\"button\" ng-click=\"paginationApi.seek(1)\" ng-disabled=\"cantPageBackward()\"><div class=\"first-triangle\"><div class=\"first-bar\"></div></div></button> <button type=\"button\" ng-click=\"paginationApi.previousPage()\" ng-disabled=\"cantPageBackward()\"><div class=\"first-triangle prev-triangle\"></div></button> <input type=\"number\" ng-model=\"grid.options.paginationCurrentPage\" min=\"1\" max=\"{{ paginationApi.getTotalPages() }}\" required> <span class=\"ui-grid-pager-max-pages-number\" ng-show=\"paginationApi.getTotalPages() > 0\">/ {{ paginationApi.getTotalPages() }}</span> <button type=\"button\" ng-click=\"paginationApi.nextPage()\" ng-disabled=\"cantPageForward()\"><div class=\"last-triangle next-triangle\"></div></button> <button type=\"button\" ng-click=\"paginationApi.seek(paginationApi.getTotalPages())\" ng-disabled=\"cantPageToLast()\"><div class=\"last-triangle\"><div class=\"last-bar\"></div></div></button></div><div class=\"ui-grid-pager-row-count-picker\"><select ng-model=\"grid.options.paginationPageSize\" ng-options=\"o as o for o in grid.options.paginationPageSizes\"></select><span class=\"ui-grid-pager-row-count-label\">&nbsp;条数据每页</span></div></div><div class=\"ui-grid-pager-count-container\"><div class=\"ui-grid-pager-count\"><span ng-show=\"grid.options.totalItems > 0\">共 {{grid.options.totalItems}} 条数据</span></div></div></div>";
		    ...
		    $scope.fileterText='';
			 ...
			 {
				 $scope.selectViewTreeKey = viewTreeCondition.viewTreeKey;
			 }
...
.directive("dcolumnfilter",['$rootScope','$timeout',"dialogs","$templateCache",
                        function($rootScope,$timeout,dialogs,$templateCache)
{
	return{
		restrict:'E',
		scope:{
		...
		{ 
			...
			this.getAdvancedByConditions = function()
			{
				...
				var params  = $scope.advancedParams;
			...
			$scope.$watch('$parent["'+$scope.gridName+'"]', function(newValue, oldValue) {  
				...
				$scope.$watch('gridController.attributes', function(newValue, oldValue) {
					if($scope.searchHolder != undefined && $scope.searchHolder !== ''){
						$scope.searchHolder = angular.fromJson($scope.searchHolder);
					}
					$scope.attributes =[];
					if(newValue != undefined){
						...
						angular.forEach(newValue, function(data,index,array){
							if(data.attributeName != "EId"){
								...
								if(firstIn){
									if(typeof $scope.searchHolder == 'object'){
										$scope.placeholder = $scope.searchHolder[data.attributeValue]?$scope.searchHolder[data.attributeValue]:'';
									}
									$scope.selected=data;
									firstIn=false;
								}
		    ...
		    $scope.fileterText='';
			  ...
			  $scope.selectOption = function(){
				  if(typeof $scope.searchHolder == 'object'){
					  $scope.placeholder = $scope.searchHolder[$scope.selected.attributeValue];
				  }
			  }
			 ...
			 $scope.advancedSearch = function(){
				 var template = 'compents/ddatagrid/asset/advancedSearchTemplate.html';
				 var dialog = dialogs.create(template,'AdvancedSearchCtr',{conditions:$scope.advancedSearchConditions},{size:'md'});
				 dialog.result.then(function(advancedParams) {
					 $scope.advancedParams = advancedParams;
					 $scope.gridController.refreshAll();
					 $scope.advancedParams = null;
				});
			 }
		},
...
.controller("AdvancedSearchCtr",['$scope','data', '$uibModalInstance',function($scope,data,$uibModalInstance) {
	$scope = angular.extend($scope,data); 
	$scope.cancelClick = function() {
	    	$(".dialogs-default").fadeOut();
	    	$(".dialogs-backdrop-default").fadeOut();
	    	$uibModalInstance.dismiss('Canceled');
	    };
	    
	$scope.rest = function(){
		for(var i = 0;i<$scope.conditions.length;i++){
			$scope.conditions[i].value = undefined;
		}
	}
	$scope.advancedSearch = function(){
		...
		for(var i=0;i<conditions.length;i++)
		 {
			if(conditions[i].type=="textRange"||conditions[i].type=="dateRange"){
				if(conditions[i].value){
					var tempValueFrom = conditions[i].value.from;
					var tempValueTo = conditions[i].value.to;
					params[conditions[i].column+"From"]= tempValueFrom==""?null:tempValueFrom;
					params[conditions[i].column+"To"]= tempValueTo==""?null:tempValueTo;
				}
			}
			else{
				var tempValue = conditions[i].value;
				params[conditions[i].column] = tempValue==""?null:tempValue;
			}
		 }
		var template  = 'compents/ddatagrid/asset/advancedSearchTemplate.html';
