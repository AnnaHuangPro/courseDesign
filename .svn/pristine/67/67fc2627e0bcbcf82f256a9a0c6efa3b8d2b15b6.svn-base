...
public class MemberGroupAction {
	...
	public MemberGroup saveMemberGroup(MemberGroup memberGroup) {
		try {
			...
			EntityUtil.loadLazyProperty(saveMemberGroup, "organization");
			EntityUtil.loadLazyProperty(saveMemberGroup, "organization.name");
	...
	public int deleteMemberGroup(MemberGroup memberGroup) {
		...
		try {
			return this.memberGroupService.deleteMemberGroup(memberGroup);
		} catch (DDDException e) {
			throw e;
		}
	...
	public Map<String, Object> saveMemberProjection(Member member, MemberGroup memberGroup,
			EntitySet<MemberType> memberTypes) {
		try {
	...
	public MemberProjection findMemberProjectionById(Long memberProjectionId) {
		try {
			MemberProjection findMemberProjection = this.memberGroupService
					.findMemberProjectionById(memberProjectionId);
	...
	public Map<String, Object> updateMemberProjection(Member member, MemberGroup memberGroup,
			EntitySet<MemberType> memberTypes) {
		try {
			Map<String, Object> result = new HashMap<String, Object>();
	...
	public LoginUser changeGroupProjection(Long memberGroupEId) {
		try {
			...
			return this.memberGroupService.changeGroupProjection(memberGroupEId);
		} catch (DDDException e) {
			throw e;
		}
	}
	...
	//查询获取会员组下的所有角色（其实角色是挂在会员类型下的）
	public MemberType findRolesByGroup(Long memberGroupId) throws Exception {
		try {
			return this.memberTypeService.findMemberTypeByGroupId(memberGroupId);
		} catch (DDDException e) {
			throw e;
		}
	}
	
	//保存会员组的角色（保存会员组默认的会员类型）
	public MemberType saveMemberGroupRoles(MemberType memberType) {
		try {
			return this.memberTypeService.saveMemberType(memberType);
		} catch (DDDException e) {
			throw e;
		}
	}
