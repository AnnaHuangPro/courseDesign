...
public class EntityClass<T extends Entity> {
	...
	public void preInit(){
		...
		for (Field field : fields) {
			...
			if(field.isAnnotationPresent(Column.class)){
				ColumnInfo columnInfo = toColumnInfo(field,methods);
	...
	public void createOne2ManySelectSql(){
		...
		for(ColumnInfo columnInfo : this.one2ManyFieldColumnInfos.values())
		{
			...
			{
				if(SessionFactory.isSqlServer()){
					String tab1 = columnInfo.getJoinTableName();
					String tab2 = manySideEntityClass.getEntityInfo().getName();
					sql = "select "+tab2+".* from "+tab1 
							+" left outer join " + tab2 
							+ " on "+tab1+"."+columnInfo.getJoinTableManySide()+" = "+tab2+"."+entityClass.getIdColumnInfo().getName()
							+" where "+tab1+"."+columnInfo.getJoinTableOneSide()+ " =? " ;
					sql += " order by "+tab1+".displayIndex ";
				}else{
	...
	private ColumnInfo toColumnInfo(Field field,Method[] methods){
		...
		if(Collection.class.isAssignableFrom(field.getType()))
		{
			...
			columnInfo.setClazz((Class) genericType.getActualTypeArguments()[0]);
		}
	...
	public PreparedStatement fillInsertStatement(PreparedStatement preparedStatement,Entity entity) throws 
		...
		if(entity.getClass().equals(this.clazz)){
			...
			for (ColumnInfo columnInfo: this.columnInfos.values()) {
				...
				if(columnInfo.getType() == ColumnInfo.COLUMN_TYPE_ONE2ONE)//如果该列是外键
				{
					...
					index++;
				}else if(columnInfo.getType() == ColumnInfo.COLUMN_TYPE_PRIMARY){//普通属性列
					Object value = methodGetter.invoke(entity, new Object[]{});
